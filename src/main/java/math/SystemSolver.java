...
public class SystemSolver {
    ...
    public static double[] solveSystem(LinearSystem system) {
        ...
        for (int i = 0; i < freeMembers.length - 1; i++) {
            boolean flag = true;
            if (matrix[i][i] == 0) {
                flag = false;
                for (int j = i + 1; j < freeMembers.length; j++) {
                    if (matrix[i][j] != 0) {
            ...
            if (!flag) return null;
            //в уравнениях i+1 .. n
            for (int k = i + 1; k < freeMembers.length; k++) {
        ...
        //ОБРАТНЫЙ ХОД
        double[] resultVector = new double[freeMembers.length];
        for (int i = freeMembers.length - 1; i > -1; i--) {
            double s = 0;
            for (int j = i + 1; j < freeMembers.length; j++) {
